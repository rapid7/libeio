...
{
	printf("res_cb(%d|%s) = %zd\n", req->type, req->data ? (char *)req->data : "?",
		EIO_RESULT(req));
...
int read_cb(eio_req * req)
{
	...
	printf("read_cb = %zd (%02x%02x%02x%02x %02x%02x%02x%02x)\n",
		EIO_RESULT(req), buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
...
int main(void)
{
	...
	do {
		...
		eio_rename("eio-test-dir/eio-test-file", "eio-test-dir/eio-test-file-renamed", 0, res_cb,
			"rename");
		event_loop();
